...
Most delivery time is not work — it is waiting. Flow-efficiency studies of knowledge work routinely find items actively worked on only **5–15%** of their elapsed time; the rest is queues. That means the cheapest acceleration is queue removal, not hiring — precisely the insight hospital patient-flow programs discovered about beds. For anything with a [cost of delay](../cost-of-delay/), flow metrics locate where the delay cost accrues.
